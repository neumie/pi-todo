...
describe("todo text", () => {
	it("sanitizes terminal controls while preserving one intent", () => {
		...
		assert.deepEqual(normalizeTodoText("\u202E safe"), {
			ok: true,
			text: "safe",
		});
	...
	it("rejects text over the documented character limit", () => {
		assert.deepEqual(normalizeTodoText("x".repeat(MAX_TODO_TEXT_CHARS + 1)), {
			ok: false,
			reason: "too-long",
		});
...
describe("todo state deltas", () => {
	it("applies legal transitions and enforces one active item", () => {
		...
		const unchanged = applyTodoMutation(state, {
			version: 1,
			op: "activate",
			id: 2,
		});
		...
		assert.deepEqual(
			state.items.map(({ id, status }) => ({ id, status })),
			[
				{ id: 1, status: "active" },
				{ id: 2, status: "queued" },
			],
		...
		assert.deepEqual(state.items, [
			{ id: 1, text: "todo 1", status: "queued" },
		]);
	});
	...
	it("strictly rejects malformed, old, foreign, oversized, and hostile entries", () => {
		...
		assert.equal(
			parseTodoMutation({ version: 2, op: "add", id: 1, text: "x" }),
			undefined,
		);
		assert.equal(
			parseTodoMutation({
		...
		assert.equal(
			parseTodoMutation({ version: 1, op: "add", id: 0, text: "x" }),
			undefined,
		);
		assert.equal(
			parseTodoMutation({ version: 1, op: "add", id: 1, text: "x\n" }),
			undefined,
		);
		assert.equal(
			parseTodoMutation({
				...
				op: "add",
				id: 1,
				text: "x".repeat(MAX_TODO_TEXT_CHARS + 1),
			}),
			undefined,
		);
